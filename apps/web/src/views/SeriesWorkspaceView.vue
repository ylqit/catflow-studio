    ...
    <template v-else-if="series">
      ...
      <section id="setting" class="card studio-section">
        ...
        <details v-if="series.adaptationPolicy === 'condense_mainline' && !activePlan" class="goal-editor">
          ...
          <label>每集时长（秒）<input v-model.number="goalDuration" type="number" :min="productionCapabilities.minimumWorkSeconds" :max="productionCapabilities.maximumWorkSeconds" /></label>
          ...
          <button class="secondary" :disabled="actionBusy || jobRunning || segmentJobRunning || !goalTitle.trim() || goalCount < 2 || goalDuration < productionCapabilities.minimumWorkSeconds || goalDuration > productionCapabilities.maximumWorkSeconds" @click="saveProductionGoal">保存生产目标</button>
        </details>
