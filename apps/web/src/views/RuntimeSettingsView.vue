...
const workerStatusLabel = computed(() => ({
  ...
  restarting: "正在恢复",
  degraded: "需要检查",
}[runtime.value?.worker.state ?? "offline"]));
...
<template>
  <main class="page settings-page">
    ...
    <div v-if="runtime" class="settings-grid">
      <section class="card settings-card"><p class="eyebrow">运行状态</p><h2>本机服务</h2><dl><div><dt>Web 服务</dt><dd><span class="pill good">可用</span></dd></div><div><dt>数据库</dt><dd><span class="pill" :class="{ good: runtime.databaseReady }">{{ runtime.databaseReady ? "可用" : "不可用" }}</span></dd></div><div><dt>后台任务</dt><dd><span class="pill" :class="{ good: runtime.worker.ready }">{{ workerStatusLabel }}</span></dd></div><div><dt>视频工具</dt><dd><span class="pill" :class="{ good: runtime.ffmpegReady && runtime.ffprobeReady }">{{ runtime.ffmpegReady && runtime.ffprobeReady ? "可用" : "不可用" }}</span></dd></div></dl><button class="secondary runtime-refresh" @click="load">重新检查</button><details class="advanced-details"><summary>查看连接信息</summary><p>正式地址：<span data-testid="runtime-address">{{ runtime.baseUrl }}</span></p><p>数据库：PostgreSQL · 后台任务：Worker · 视频工具：FFmpeg / ffprobe</p><p v-if="runtime.worker.lastHeartbeatAt">后台任务最后心跳：{{ new Date(runtime.worker.lastHeartbeatAt).toLocaleString() }}</p><p v-if="runtime.worker.lastExitAt">最近退出：{{ new Date(runtime.worker.lastExitAt).toLocaleString() }}</p><p>自动重启次数：{{ runtime.worker.restartCount }}</p></details></section>
...
.runtime-refresh { width: 100%; margin-top: 14px; }
