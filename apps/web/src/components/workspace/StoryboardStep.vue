...
const comparisonSummary = computed(() => {
  ...
  const coreFields: Array<keyof ShotSpecDto> = [
    "durationSeconds", "framing", "cameraMovement", "childAction", "catAction", "environmentChange", "transition",
  ];
  let changed = 0;
  for (let index = 0; index < Math.min(left.shots.length, right.shots.length); index += 1) {
    for (const field of coreFields) {
      if (left.shots[index][field] !== right.shots[index][field]) changed += 1;
    }
  }
  return {
...
function coreFieldChanged(index: number, field: keyof ShotSpecDto) {
  const current = activePlan.value?.shots[index];
  const compared = selectedPlan.value?.shots[index];
  return !current || !compared || current[field] !== compared[field];
}
  ...
  <section v-else class="storyboard-layout">
        ...
        <article v-for="shot in shots" :key="shot.id" class="shot-card">
          <div class="shot-summary">
            ...
            <div class="shot-fields">
              ...
              <div class="field wide"><label>人物动作</label><input v-model="shot.childAction" :disabled="!canEditSelected" /></div><div class="field wide"><label>猫咪动作</label><input v-model="shot.catAction" :disabled="!canEditSelected" /></div>
              <div class="field wide"><label>画面变化</label><input v-model="shot.environmentChange" :disabled="!canEditSelected" /></div><div class="field compact"><label>转场</label><select v-model="shot.transition" :disabled="!canEditSelected"><option value="continuous">连续</option><option value="soft_cut">柔切</option><option value="hard_cut">硬切</option></select></div>
            </div>
          </div>
    ...
    <div v-if="compareOpen && activePlan && selectedPlan" class="compare-backdrop" @click.self="closeComparison">
      <aside data-testid="shot-plan-compare-drawer" class="compare-drawer" role="dialog" aria-modal="true" aria-labelledby="compare-title">
        ...
        <details class="professional-compare"><summary>查看专业差异</summary><pre>{{ JSON.stringify({ current: activePlan.shots, compared: selectedPlan.shots }, null, 2) }}</pre></details>
      </aside>
    </div>
  </section>
</template>
...
@media (max-width: 1050px) { .storyboard-layout { grid-template-columns: 1fr; }.story-source { position: static; }.professional-grid, .compare-columns { grid-template-columns: 1fr; }.professional-grid .span-two { grid-column: auto; }.editor-head { align-items: flex-start; }.progress-steps { grid-template-columns: 1fr; } }
</style>
